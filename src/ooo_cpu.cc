...
void O3_CPU::operate()
{
  if (!IFETCH_BUFFER.front().checked) {
    ...
    if (IFETCH_BUFFER.front().fetched != COMPLETED && !IFETCH_BUFFER.front().uop_cache_hit) { // FTQ head is not fetched so it will stall
  ...
  if ((fetch_stall == 1) && (current_cycle < fetch_resume_cycle)) { // fetch stoped due to mispredicted branch
...
void O3_CPU::initialize_instruction()
{
  ...
  } else { // if it is not branch miss then just add number of instructions possible depending on the size of the FTQ
  ...
  } else { // do not issue demand from conflict banks
...
void O3_CPU::check_contention()
{
  ...
  auto demand_begin_ip = std::find_if(std::begin(demand_ips), std::end(demand_ips), [](const demand_ip_info& x) { return !x.btb_contention_checked; });
  uint8_t alternate_fetch_width = FETCH_WIDTH;
  while (alternate_fetch_width > 0) {
    if (demand_ips.empty()) {
      break;
    }
    auto demand_begin = std::find_if(std::begin(demand_ips), std::end(demand_ips), [](const demand_ip_info& x) { return !x.btb_contention_checked; });
  ...
  auto pref_begin = std::find_if(std::begin(ips_to_prefetch), std::end(ips_to_prefetch), [](const ip_pref_entry& x) { return !x.btb_checked; });
  ...
  while (alternate_fetch_width > 0) {
    ...
    // if bank is not used then only use it
    {
      if (!demand_banks[EXTRACT_BITS((pref_begin->ip >> 2), lg2(BTB_BANKS))]) {
        ...
        btb_conflict_free = true;
      } else if (pref_begin->is_head && pref_begin->last_cycle_to_try_btb != current_cycle) { // conflict so update the probability
  ...
  while (dib_ports > 0) {
    ...
    auto demand_begin = std::find_if(std::begin(demand_ips), std::end(demand_ips), [](const demand_ip_info& x) { return !x.dib_contention_checked; });
  ...
  dib_ports = 2;
  while (dib_ports > 0) {
    auto pref_begin = std::find_if(std::begin(ips_to_prefetch), std::end(ips_to_prefetch), [](const ip_pref_entry& x) { return !x.dib_check_done; });
...
void O3_CPU::increment_alternate_path()
{
  ...
  while (alternate_fetch_width > 0 && !ips_to_prefetch.empty()) {
    ...
    if (should_add) {
      ...
      for (auto i = pref_request; i != pref_end; i++) { // add the ips to prefetch
...
#define NEW_TRACE
  ...
  } else if (!reads_sp && !reads_ip && !reads_flags && writes_ip && reads_other) { // } else if (!reads_sp && !reads_flags && writes_ip && reads_other) {
    // indirect branch
    arch_instr.is_branch = true;
    arch_instr.branch_taken = true;
    arch_instr.branch_type = BRANCH_INDIRECT;
  } else if (!reads_sp && reads_ip && !writes_sp && writes_ip && (reads_flags || reads_other)) { // conditional branch
    arch_instr.is_branch = true;
  ...
  } else if (reads_sp && reads_ip && writes_sp && writes_ip && !reads_flags && reads_other) { // indirect call
    arch_instr.is_branch = true;
    arch_instr.branch_taken = true;
  ...
  } else if (writes_ip) { // some other branch type that doesn't fit the above categories
    arch_instr.is_branch = true;
    arch_instr.branch_taken = arch_instr.branch_taken; // don't change this
  ...
  if (arch_instr.is_branch) {
    ...
    if (arch_instr.alt_predicted_target != arch_instr.branch_target
        || (arch_instr.branch_type == BRANCH_CONDITIONAL && arch_instr.branch_taken != arch_instr.alt_prediction)) {
    ...
    if (arch_instr.predicted_target != arch_instr.branch_target
        || (arch_instr.branch_type == BRANCH_CONDITIONAL
    ...
    if (arch_instr.hard_to_predict_branch && arch_instr.branch_type == BRANCH_CONDITIONAL) {
      ...
      if (arch_instr.branch_prediction) { // taken so prefetch not-taken path
    ...
    if (!last_br_taken) { // not taken bb_size does not cost a btb entry
  ...
  if (begin_branch_recovery) {
    if (arch_instr.is_branch) { //&& arch_instr.branch_taken) { //counting all branches
  ...
  if (begin_branch_recovery) {
    assert(last_br_miss_type != 0);
    if (last_br_miss_type == BRANCH_CONDITIONAL || (last_br_miss_type == BRANCH_INDIRECT || last_br_miss_type == BRANCH_INDIRECT_CALL)
        || last_br_miss_type == BRANCH_RETURN) {
      total_critical_instruction++;
      arch_instr.is_critical = true;
    }
  }
  ...
  if (begin_branch_recovery) {
    assert(last_br_miss_type != 0);
    if (last_br_miss_type == BRANCH_CONDITIONAL || (last_br_miss_type == BRANCH_INDIRECT || last_br_miss_type == BRANCH_INDIRECT_CALL)
        || last_br_miss_type == BRANCH_RETURN || (last_br_miss_type == BRANCH_DIRECT_CALL || last_br_miss_type == BRANCH_DIRECT_JUMP)) {
      total_critical_instruction++;
      arch_instr.is_critical = true;
    }
  }
  ...
  if (begin_branch_recovery) {
    assert(last_br_miss_type != 0);
    if (last_br_miss_type == BRANCH_CONDITIONAL || (last_br_miss_type == BRANCH_INDIRECT || last_br_miss_type == BRANCH_INDIRECT_CALL)
        || (last_br_miss_type == BRANCH_DIRECT_CALL || last_br_miss_type == BRANCH_DIRECT_JUMP)) {
      total_critical_instruction++;
      arch_instr.is_critical = true;
    }
  }
...
#define CP_H2P_PATH
    ...
    if (br_type > 0 && m_prediction && m_target > 0) { // predicted taken branch
      m_ip = m_target;
    } else {
      m_ip += 4;
    }
    ...
    if (br_type == BRANCH_DIRECT_CALL || br_type == BRANCH_DIRECT_JUMP || br_type == BRANCH_INDIRECT || br_type == BRANCH_INDIRECT_CALL
        || br_type == BRANCH_OTHER || br_type == BRANCH_RETURN) {
      num_br++;
    }
...
void O3_CPU::decode_prefetched_instruction()
{
  ...
  while (available_decode_bandwidth > 0 && !PRF_DECODE_BUFFER.empty()) {
    ...
    if (PRF_DECODE_BUFFER.front().br_type == BRANCH_CONDITIONAL || PRF_DECODE_BUFFER.front().br_type == BRANCH_DIRECT_CALL
        || PRF_DECODE_BUFFER.front().br_type == BRANCH_DIRECT_JUMP || PRF_DECODE_BUFFER.front().br_type == BRANCH_INDIRECT
        || PRF_DECODE_BUFFER.front().br_type == BRANCH_INDIRECT_CALL || PRF_DECODE_BUFFER.front().br_type == BRANCH_OTHER
        || PRF_DECODE_BUFFER.front().br_type == BRANCH_RETURN) {
      is_br = true;
    }
...
{
  while (!static_cast<CACHE*>(L1I_bus.lower_level)->UOP_CACHE_MSHR.empty() && static_cast<CACHE*>(L1I_bus.lower_level)->UOP_CACHE_MSHR.front().pref_done
         && static_cast<CACHE*>(L1I_bus.lower_level)->UOP_CACHE_MSHR.front().branch_resolved) {
  ...
  // check for deadlock
  if (!std::empty(static_cast<CACHE*>(L1I_bus.lower_level)->UOP_CACHE_MSHR)
      && (static_cast<CACHE*>(L1I_bus.lower_level)->UOP_CACHE_MSHR.front().event_cycle + UOP_MSHR_DEADLOCK_CYCLE) <= current_cycle) {
...
void O3_CPU::check_dib()
{
  ...
  while (dib_ports > 0) {
    ...
    auto dib_begin = std::find_if(std::begin(IFETCH_BUFFER), std::end(IFETCH_BUFFER), [](const ooo_model_instr& x) { return !x.dib_checked; });
...
{
  if (instr.event_cycle < current_cycle && std::size(ROB) != ROB_SIZE
      && ((std::size_t)std::count_if(std::begin(LQ), std::end(LQ), std::not_fn(is_valid<decltype(LQ)::value_type>{})) >= std::size(instr.source_memory))
      && ((std::size(instr.destination_memory) + std::size(SQ)) <= SQ_SIZE)) {
...
void O3_CPU::stream_instruction()
{
  ...
  auto [uop_window_begin, uop_window_end] =
      champsim::get_span_p(std::begin(IFETCH_BUFFER), std::end(IFETCH_BUFFER), (fetch_bandwidth),
                           [cycle = current_cycle](const auto& x) { return x.uop_cache_hit && x.dib_checked == COMPLETED; });
...
void O3_CPU::update_mshr(ooo_model_instr& instr)
{
  ...
  if (instr.hard_to_predict_branch) { // h2p and not mispredicted
    if (!instr.branch_mispredicted) {
      for (auto& mshr_entry : static_cast<CACHE*>(L1I_bus.lower_level)->UOP_CACHE_MSHR) {
        if (mshr_entry.by_instr == instr.instr_id) { // invalidate all the lines as it will not be used anymore
...
void O3_CPU::handle_memory_return()
{
  for (auto l1i_bw = FETCH_WIDTH, to_read = L1I_BANDWIDTH; l1i_bw > 0 && to_read > 0 && !L1I_bus.PROCESSED.empty(); --to_read) {
    ...
    while (l1i_bw > 0 && !l1i_entry.instr_depend_on_me.empty()) {
      ...
      auto ftq_entry =
          std::find_if(std::begin(IFETCH_BUFFER), std::end(IFETCH_BUFFER), [line_id = fetched.instr_id](ooo_model_instr& x) { return x.instr_id == line_id; });
...
void O3_CPU::retire_rob()
{
  ...
  if constexpr (champsim::uop_debug_print) {
    std::for_each(retire_begin, retire_end,
                  [&](const auto& x) { std::cout << "[RETIRE] instr_id: " << x.instr_id << " is retired @" << current_cycle << std::endl; });
  }
