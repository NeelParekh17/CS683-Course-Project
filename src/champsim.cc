    ...
    while (!std::accumulate(std::begin(phase_complete), std::end(phase_complete), true, std::logical_and{})) {
      ...
      for (O3_CPU& cpu : ooo_cpu) {
        ...
        if (!phase_complete[cpu.cpu] && (cpu.sim_instr() >= length)) {
          ...
          if (phase_name.compare("Warmup") == 0) {
            get_profiler_ptr->clear_stats_after_warmup();

            cpu.begin_sim_cycle = cpu.current_cycle;
            cpu.begin_sim_instr = cpu.num_retired;

            // reset instructions per stage
            cpu.num_ifetch = 0;
            cpu.num_decode = 0;
            cpu.num_dispatch = 0;
            cpu.num_rob = 0;
            cpu.num_rob_wp = 0;
            cpu.num_rob_wp_info = 0;
            cpu.dib_accesses = 0;
            cpu.dib_hits = 0;
            cpu.num_dib_checked = 0;
            // reset events stats
            cpu.addr_gen_events = 0;
            cpu.src_regs = 0;
            cpu.dst_regs = 0;
            cpu.rob_writes = 0;
            cpu.lq_writes = 0;
