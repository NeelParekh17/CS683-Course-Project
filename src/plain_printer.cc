...
void champsim::plain_printer::print(O3_CPU::stats_type stats)
{
  ...
  get_profiler_ptr->ipc = std::ceil(stats.instrs()) / std::ceil(stats.cycles());
  get_profiler_ptr->total_cycles = stats.cycles();
  ...
  get_profiler_ptr->branch_mpki = (1000.0 * total_mispredictions) / std::ceil(stats.instrs());

  std::cerr << "MPKI " << (1000.0 * total_mispredictions) / std::ceil(stats.instrs()) << " get_profiler_ptr->branch_mpki " << get_profiler_ptr->branch_mpki
            << std::endl;

  std::cerr << "TOTAL_MISS: " << total_mispredictions << " TOTAL_INSTR: " << std::ceil(stats.instrs()) << std::endl;
...
void champsim::plain_printer::print(CACHE::stats_type stats)
{
  ...
  for (std::size_t cpu = 0; cpu < NUM_CPUS; ++cpu) {
    ...
    if (stats.name.length() >= 3 && stats.name.compare(stats.name.length() - 3, 3, "L1I") == 0) {
      get_profiler_ptr->l1i_hit = TOTAL_HIT;
      get_profiler_ptr->l1i_miss = TOTAL_MISS;
      get_profiler_ptr->l1i_access = TOTAL_HIT + TOTAL_MISS;
    }
    ...
    get_profiler_ptr->add_to_stats(stats.name, "ACCESS", TOTAL_HIT + TOTAL_MISS);
    get_profiler_ptr->add_to_stats(stats.name, "HIT", TOTAL_HIT);
    get_profiler_ptr->add_to_stats(stats.name, "MISS", TOTAL_MISS);
    ...
    for (const auto& type : types) {
      ...
      get_profiler_ptr->add_to_stats(stats.name, name.str() + "_ACCESS", stats.hits[type.second][cpu] + stats.misses[type.second][cpu]);
      get_profiler_ptr->add_to_stats(stats.name, name.str() + "_HIT", stats.hits[type.second][cpu]);
      get_profiler_ptr->add_to_stats(stats.name, name.str() + "_MISS", stats.hits[type.second][cpu]);
    }
...
void champsim::plain_printer::print(DRAM_CHANNEL::stats_type stats)
{
  ...
  // stream << "UOP_CACHE_HIT: " << double(get_profiler_ptr->uop_cache_hit) / double(get_profiler_ptr->uop_cache_read) * 100.00;
  stream << std::endl;
}
