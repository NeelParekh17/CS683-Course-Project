...
namespace
{
  ...
  constexpr std::size_t BTB_SET = 4 * 1024;
...
std::pair<std::pair<uint64_t, uint8_t>, extra_branch_info>  O3_CPU::btb_prediction(uint64_t ip, uint8_t level) {
  ...
  if (!btb_entry.has_value())
    return std::make_pair(std::make_pair(0, false), br_info_btb);

  if (btb_entry->type == ::branch_info::RETURN) {
    if (std::empty(::RAS[this]))
      return std::make_pair(std::make_pair(0, true), br_info_btb);
    ...
    return std::make_pair(std::make_pair(target + size, true), br_info_btb);
  }

  if (btb_entry->type == ::branch_info::INDIRECT) {
    auto hash = (ip >> 2) ^ ::CONDITIONAL_HISTORY[this].to_ullong();
    return std::make_pair(std::make_pair(::INDIRECT_BTB[this][hash % std::size(::INDIRECT_BTB[this])], true), br_info_btb);
  }

  return std::make_pair(std::make_pair(btb_entry->target, btb_entry->type != ::branch_info::CONDITIONAL), br_info_btb);
}
