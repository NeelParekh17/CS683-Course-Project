...
public:
  ...
  void initialize() {
    std::cout << "H2P BTB sets: " << SETS << " ways: " << WAYS << std::endl;
  ...
  std::pair<uint64_t, uint64_t> get_bb_size(uint64_t ip) {
    ...
    //  if (ip == 0xffffb74359bc)
    // cout << __func__ << " " << std::hex << ip << " taken " << btb_entry->taken_bb_size << " nt " << btb_entry->not_taken_bb_size << std::dec << endl;
  ...
  void m_update_bb_size(uint64_t ip, uint64_t bb_size, bool taken) {
    // if (ip == 0xffffb74359bc)
    //   cout << "Update bb in simple btb " << std::hex << " " << ip << " index " << set_index(ip) << std::dec << " size " << bb_size << endl;
    auto btb_entry = find_entry(ip);
    if (btb_entry == NULL) {
      assert(false); //entry should be already in the btb
      return;
    }
  ...
  void update(uint64_t ip, uint64_t branch_target, uint8_t taken, uint8_t branch_type) {
    // if (ip == 0xffffb338af6c)
    //   cout << __func__ << std::hex << " ip " << ip << " type? " << std::dec << int(branch_type) << endl;
    auto btb_entry = find_entry(ip);
    if (btb_entry == NULL) {
