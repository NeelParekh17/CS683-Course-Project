...
namespace {
  ...
  private:
    ...
    void push_ras_bb_size(uint64_t ip, uint64_t bb_size) {
      for (int index = 0; index <= RAS_SIZE; index++) {
        if (ras[index] == ip) {
          ras_bb_size[index] = bb_size;
        }
      }
    }
  ...
  public:
    ...
    void update(uint64_t ip, uint64_t branch_target, uint8_t taken, uint8_t branch_type) {
      //cout << "Update RAS " << ip << " target " << branch_target << " taken " << int(taken) << " type " << int(branch_type) << endl;
