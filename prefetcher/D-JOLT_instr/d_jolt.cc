...
#define ShortRangePrefetcherSiggen Siggen_FifoRetCnt<4>
  ...
  class UpperBitTable {
    struct Entry {
      bool valid; uint64_t upper_bits;
    };
  ...
  public:
    ...
    std::vector<CompressedLineAddress> getAddresses() const {
      ...
      for (size_t i = 0; i < bit_vector.size(); ++i) {
        if (bit_vector.at(i)) {
          CompressedLineAddress tmp = base_address; tmp.lower_part += (i + 1); ret.push_back(tmp);
        }
      }
      return ret;
    }
  ...
  public:
    const U& operator[](const T& key) const {
      return table.at(HasherForIndex{}(key))[key];
    }
    U& operator[](const T& key) {
      return table.at(HasherForIndex{}(key))[key];
    }
    void touch(const T& key) {
      table.at(HasherForIndex{}(key)).touch(key);
    }
    void insert(const T& key, const U& elem) {
      table.at(HasherForIndex{}(key)).insert(key, elem);
    }
    bool contains(const T& key) const {
      return table.at(HasherForIndex{}(key)).contains(key);
    }
  };
  ...
  class WindowBasedStreamPrefetcher {
    ...
    bool update_training_stream_and_prefetch(uint64_t line_address, CACHE* m_cache) {
      for (size_t i = 0; i < TrainingTableSize; ++i) {
        ...
        if (stream.count >= TrainingThreshold) {
          prefetch_initial_stream(line_address, stream, m_cache);
    ...
    bool update_monitoring_stream_and_prefetch(uint64_t line_address, CACHE* m_cache) {
      for (size_t i = 0; i < MonitoringTableSize; ++i) {
        ...
        for (size_t j = 0; j < Degree; ++j) {
          ...
          if (pf_addr > 0)
            m_cache->prefetch_line(pf_addr, true, 0);
    ...
    void prefetch_initial_stream(uint64_t line_address, const TrainingStreamEntry& stream, CACHE* m_cache) {
  ...
  public:
    void cache_operate(uint64_t address, uint8_t cache_hit, uint8_t prefetch_hit, CACHE* m_cache) {
      const bool virtual_miss = cache_hit == 0 || prefetch_hit == 1;
      const uint64_t line_address = address >> LOG2_BLOCK_SIZE;

      const bool already_monitored_stream = update_monitoring_stream_and_prefetch(line_address, m_cache);

      if (virtual_miss) {
        const bool already_training_stream = update_training_stream_and_prefetch(line_address, m_cache);
  ...
  public:
    uint32_t back() const noexcept {
      return queue.at((head + 1) % N);
    }
    void insert(uint32_t x) noexcept {
      queue.at((head + 1) % N) = x; head = (head + 1) % N;
    }
  };
  ...
  class D_JOLT_PREFETCHER {
    CACHE* m_cache;
    struct SigHasher {
      size_t operator()(const uint32_t& x) const noexcept {
        return x;
      }
    };
  ...
  public:
    D_JOLT_PREFETCHER(CACHE* m_cache);
  ...
  D_JOLT_PREFETCHER::D_JOLT_PREFETCHER(CACHE* m_cache) : m_cache(m_cache) {
    ...
    if (table.contains(sig)) {
      for (const auto& v : table[sig].getValidEntries()) {
        for (const auto& address : v.getAddresses()) {
          ...
          if (pf_addr > 0)
            const int success = m_cache->prefetch_line(pf_addr, true, 0);
        }
      }
    }
  }
  ...
  void D_JOLT_PREFETCHER::cache_operate(uint64_t addr, uint8_t cache_hit, uint8_t prefetch_hit) {
    ...
    const bool virtual_miss = cache_hit == 0 || prefetch_hit == 1;
    stream_prefetcher.cache_operate(addr, cache_hit, prefetch_hit, m_cache);
...
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in){
  ::l1i_prefetcher.at(cpu)->cache_operate(ip, cache_hit, type);
  return metadata_in;
}
...
{
  ::l1i_prefetcher.at(cpu)->cache_fill(addr, set, way, prefetch, evicted_addr);
  return metadata_in;
}
