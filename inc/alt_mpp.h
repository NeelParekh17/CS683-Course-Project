 ...
 * Daniel A. Jimenez
 *
 * This file simulates the 8.25KB and a 64KB "multiperspective perceptron predictor" for CBP2015.
 *
 */

#ifndef _MBP_PREDICTOR_H_
#define _MBP_PREDICTOR_H_
...
namespace alt_mbp
{
class branch_info_mdp
{
public:
  bool m_prediction;
  bool spec_prediction;
  unsigned int address, spec_address;

  branch_info_mdp(void) {}

  // set the prediction for this branch

  void prediction(bool p) { m_prediction = p; }
  ...
  bool prediction(void) { return m_prediction; }
};
...
public:
  ...
  // the return value is a pointer to a branch_info_mdp object that gives
  // the prediction and presumably contains information needed to
  // update the predictor

  virtual std::pair<branch_info_mdp*, int> lookup(unsigned int pc, uint64_t full_pc) = 0;
  ...
  virtual void update(branch_info_mdp* p, unsigned int target, bool taken, int type = 0, uint64_t pc = 0) = 0;
  ...
  virtual void SaveProcessorState() {}
  virtual void RestoreProcessorState() {}
...
class multiperspective_perceptron_update : public branch_info_mdp
{
public:
  unsigned int pc, spec_pc;
  unsigned short int hpc, pc2, spec_hpc, spec_pc2;
  int yout, spec_yout;
  int indices[MAX_TABLES], spec_indices[MAX_TABLES];
  bool filtered, spec_filtered;
  ...
  void SaveStateUpdate()
  {
    ...
    spec_filtered = filtered;
  }

  void RestoreStateUpdate()
  {
    pc = spec_pc;
    hpc = spec_hpc;
    pc2 = spec_pc2;
    yout = spec_yout;
    for (int i = 0; i < MAX_TABLES; i++) {
      indices[i] = spec_indices[i];
    }
    filtered = spec_filtered;
  }
...
// feature types
enum history_type {
  MBP_GHIST = 1,     // global pattern history
...
inline int paircmp(const bestpair* a, const bestpair* b) { return a->mpreds - b->mpreds; }
...
inline history_spec good3_mbp[16] = {
    {BIAS, -1, -1, -1, 2.40625, 0, 6},       {MBP_GHIST, 0, 19, 1, 1.4375, 0, 6},      {MBP_GHIST, 0, 65, 1, 1.0, 0, 6},
    {MBP_GHIST, 21, 64, 1, 1.0, 0, 6},       {MBP_GHIST, 75, 150, 1, 1.0625, 0, 6},    {GHISTMODPATH, 0, 7, 3, 1.625, 0, 6},
    {GHISTPATH, 11, 2, -1, 1.25, 0, 6},      {GHISTPATH, 15, 4, -1, 1.125, 0, 6},      {GHISTPATH, 31, 1, -1, 1.40625, 0, 6},
    {GHISTPATH, 7, 1, -1, 1.500000, 600, 6}, {IMLI, 4, -1, -1, 1.28125, 375, 6},       {LOCAL, -1, -1, -1, 1.5625, 512, 6},
    {RECENCY, 14, 4, -1, 1.250000, 0, 6},    {RECENCYPOS, 31, -1, -1, 1.875000, 0, 6}, {SGHISTPATH, 0, 4, 3, 1.65625, 0, 6},
    {SGHISTPATH, 1, 2, 5, 2.531250, 0, 5},
};

// tuned features for 645KB predictor

inline history_spec big[37] = {
    {ACYCLIC, 10, -1, -1, 1.0, 0, 6},      {BLURRYPATH, 10, 7, -1, 1.0, 0, 6},  {MBP_GHIST, 0, 19, 1, 1.3125, 0, 6},
    {MBP_GHIST, 0, 65, 1, 0.850000, 0, 6}, {MBP_GHIST, 111, 222, 1, 1.0, 0, 6}, {MBP_GHIST, 115, 206, 1, 1.0, 0, 6},
    {MBP_GHIST, 190, 336, 1, 1.0, 0, 6},   {MBP_GHIST, 21, 64, 1, 1.0, 0, 6},   {MBP_GHIST, 48, 119, 1, 1.0, 0, 6},
    {MBP_GHIST, 67, 203, 1, 0.75, 0, 6},   {MBP_GHIST, 75, 150, 1, 1.0, 0, 6},  {GHISTMODPATH, 1, 5, 4, 1.450000, 1913, 6},
    {GHISTMODPATH, 3, 13, 1, 1.0, 0, 6},   {GHISTPATH, 105, 4, 0, 1.0, 0, 6},   {GHISTPATH, 11, 2, -1, 1.25, 0, 6},
    {GHISTPATH, 15, 4, -1, 1.125, 0, 6},   {GHISTPATH, 23, 4, -1, 1.0, 0, 6},   {GHISTPATH, 31, 1, -1, 1.0, 0, 6},
    {GHISTPATH, 32, 2, -1, 1.0, 0, 6},     {GHISTPATH, 36, 4, -1, 1.0, 0, 6},   {GHISTPATH, 51, 1, 0, 1.0, 0, 6},
    {GHISTPATH, 7, 1, -1, 1.5, 0, 6},      {GHISTPATH, 72, 1, -1, 1.0, 0, 6},   {GHISTPATH, 86, 4, 0, 1.0, 0, 6},
    {IMLI, 1, -1, -1, 1.8125, 0, 6},       {IMLI, 4, -1, -1, 1.78125, 1536, 6}, {LOCAL, -1, -1, -1, 1.0, 1024, 6},
    {LOCAL, -1, -1, -1, 2.0, 3467, 6},     {MODHIST, 1, 16, -1, 0.9375, 0, 6},  {MODPATH, 3, 20, 1, 1.0, 0, 6},
    {PATH, 13, 2, -1, 1.4375, 0, 6},       {PATH, 27, 5, -1, 1.00000, 0, 6},    {RECENCY, 10, 3, -1, 0.550000, 1024, 6},
    {RECENCY, 14, 4, -1, 1.3125, 0, 6},    {RECENCYPOS, 31, -1, -1, 1.5, 0, 6}, {SGHISTPATH, 1, 2, 5, 1.25, 768, 6},
...
#define MAX_ASSOC 256
  ...
  unsigned int my_seed, spec_my_seed;

  // branch filter

  filter_entry filter_table[MAX_FILTER], spec_filter_table[MAX_FILTER];

  // table of per-branch (local) histories

  unsigned int local_histories[MAX_LOCAL_HISTORIES], spec_local_histories[MAX_LOCAL_HISTORIES];

  // counter for adaptive theta setting

  int tc, spec_tc;

  int occupancy, spec_occupancy;

  // tables of weight magnitudes

  weight tables[MAX_TABLES][MAX_TABLE_SIZE], spec_tables[MAX_TABLES][MAX_TABLE_SIZE];

  // tables of weight signs

  bool sign_bits[MAX_TABLES][MAX_TABLE_SIZE][2], spec_sign_bits[MAX_TABLES][MAX_TABLE_SIZE][2];

  // global history bits divided into block_size bits per array element

  unsigned int ghist_words[MAX_GHIST / MAX_LG_TABLE_SIZE + 1], spec_ghist_words[MAX_GHIST / MAX_LG_TABLE_SIZE + 1];

  // count of mispredictions per table

  int mpreds[MAX_TABLES], spec_mpreds[MAX_TABLES];

  // acyclic histories

  bool acyclic_histories[MAX_ACYCLIC][32], spec_acyclic_histories[MAX_ACYCLIC][32];

  // alternate acyclic histories that use path instead of pattern history

  unsigned int acyclic2_histories[MAX_ACYCLIC][32], spec_acyclic2_histories[MAX_ACYCLIC][32];

  // modulo pattern history
  bool mod_histories[MAX_MOD][MAX_GHIST], spec_mod_histories[MAX_MOD][MAX_GHIST];

  // modulo path history

  unsigned short int modpath_histories[MAX_MOD][MAX_PATHHIST], spec_modpath_histories[MAX_MOD][MAX_PATHHIST];

  // "page" history

  unsigned int blurrypath_histories[MAX_BLURRY][MAX_BLURRY2], spec_blurrypath_histories[MAX_BLURRY][MAX_BLURRY2];

  // recency stack of recenctly visited PCs (hashed)

  unsigned int short recency_stack[MAX_ASSOC], spec_recency_stack[MAX_ASSOC];

  // the last global history outcome

  bool last_ghist_bit, spec_last_ghist_bit;

  // history of recent PCs (hashed)

  unsigned int short path_history[MAX_PATHHIST], spec_path_history[MAX_PATHHIST];

  // innermost loop iteration counters; 4 versions

  unsigned int imli_counter1, imli_counter2, imli_counter3, imli_counter4;
  unsigned int spec_imli_counter1, spec_imli_counter2, spec_imli_counter3, spec_imli_counter4;
  ...
  int
      ...
      modpath_indices[MAX_MOD], modpath_lengths[MAX_MOD], modhist_lengths[MAX_MOD],

      spec_modhist_indices[MAX_MOD], spec_modpath_indices[MAX_MOD], spec_modpath_lengths[MAX_MOD], spec_modhist_lengths[MAX_MOD];
  ...
  int nmodhist_histories, nmodpath_histories, spec_nmodhist_histories, spec_nmodpath_histories;
  ...
  history_spec* specs;
  int num_tables,           // number of features (each one gets its own table)
  ...
  double fudge;   // fudge factor to multiply by perceptron output
  int *xlat,      // transfer function for 6-bit weights (5-bit magnitudes)
  ...
  bool hash_taken;             // should we hash the taken/not taken value with a PC bit?
  int n_sign_bits,             // number of sign bits per magnitude
  ...
  unsigned int record_mask; // which histories are updated with filtered branch outcomes

  int speed,     // speed for adaptive theta training
      threshold, // threshold for deciding low/high confidence
      tunebits;  // number of bits in misprediction counters

  bool tuneonly;   // if true, only count mispredictions of low-confidence branches
  int nbest,       // use this many of the top performing tables on a low-confidence branch
  ...
  int decay; // whether and how often to decay a random weight

  unsigned long long int imli_mask1, // which tables should have their indices hashed with the first IMLI counter
  ...
  multiperspective_perceptron(history_spec* _specs, int _num_tables = 38, int _budgetbits = 65536 * 8 + 2048, int _nlocal_histories = 512,
                              int _local_history_length = 11, int _theta = 10, double _fudge = 0.26, int* _xlat = NULL, int* _xlat4 = NULL, int _pcbit = 2,
                              int _pcshift = -10, int _block_size = 21, int _num_filter = 18000, bool _hash_taken = false, int _n_sign_bits = 2,
                              int _extra_rounds = 1, unsigned int _record_mask = 191, int _speed = 9, int _threshold = 0, int _tunebits = 30,
                              bool _tuneonly = false, int _nbest = 1, int _bias0 = 0, int _bias1 = 0, int _biasmostly0 = 0, int _biasmostly1 = 0,
                              int _hshift = 0, int _decay = 0, unsigned long long int _imli_mask1 = 0, unsigned long long int _imli_mask4 = 0,
                              unsigned long long int _recencypos_mask = 0)
      : specs(_specs), num_tables(_num_tables), budgetbits(_budgetbits), nlocal_histories(_nlocal_histories), local_history_length(_local_history_length),
        theta(_theta), fudge(_fudge), xlat(_xlat), xlat4(_xlat4), pcbit(_pcbit), pcshift(_pcshift), block_size(_block_size), num_filter(_num_filter),
        hash_taken(_hash_taken), n_sign_bits(_n_sign_bits), extra_rounds(_extra_rounds), record_mask(_record_mask), speed(_speed), threshold(_threshold),
        tunebits(_tunebits), tuneonly(_tuneonly), nbest(_nbest), bias0(_bias0), bias1(_bias1), biasmostly0(_biasmostly0), biasmostly1(_biasmostly1),
        hshift(_hshift), decay(_decay), imli_mask1(_imli_mask1), imli_mask4(_imli_mask4), recencypos_mask(_recencypos_mask)
  {
  ...
  unsigned int hash_ghistmodpath(int a, int depth, int shift)
  {
    ...
    for (int i = 0; i < depth; i++) {
      ...
      // if (!spec_state) cout << __func__ << " (modpath_histories[a][i] << 1) " << (modpath_histories[a][i] << 1) << " mod_histories[a][i] " <<
      // mod_histories[a][i] << " a " << a << " i " << i << endl;
    }
    return x;
  }
  ...
  {
    // if (!spec_state) cout << __func__ << " s->type " << s->type << endl;
    ...
    case MODHIST:
      ...
      break;
    case GHISTMODPATH:
      // if (!spec_state) cout << __func__ << " s->p1 " << s->p1 << " s->p2 " << s->p2 << " s->p3 " << s->p3 << endl;
      x = hash_ghistmodpath(s->p1, s->p2, s->p3);
      // if (!spec_state) cout << __func__ << " x " << x << endl;
      break;
    case MODPATH:
  ...
  {
    // if (!spec_state)
    //   cout << __func__ << "  u.pc " << u.pc << " u.pc2 " << u.pc2
    //   << " &specs[i] " << specs[i].coeff
    //   << " p1 " << specs[i].p1
    //   << " p2 " << specs[i].p2
    //   << "p3 " << specs[i].p3
    //   << " size " << specs[i].size
    ...
    // if (!spec_state) cout << __func__ << " g " << g << endl;
  ...
  void compute_output(int* bestval)
  {
    ...
    // if (!spec_state)
    //   cout << __func__ << " x " << x << " bias0 " << bias0 << endl;
    if (x == 0)
      u.yout = bias0;
    ...
    *bestval = 0;
    ...
    for (int i = 0; i < num_tables; i++) {
      ...
      // std::cout << __func__ << " " << h << std::endl;

      // add the weight; first get the weight's magnitude

      int c = tables[i][h];

      // std::cout << __func__ << " c " << c << " i " << i << " h " << h << std::endl;

      // get the sign

      bool sign = sign_bits[i][h][u.hpc % n_sign_bits];
      // if (!spec_state)
      //   cout << __func__ << " sign " << sign << " i " << i << " u.hpc % n_sign_bits " << (u.hpc % n_sign_bits) << " h " << h << endl;
      ...
      // std::cout << __func__ << " " << *bestval << " u.yout " << u.yout << " threshold " << threshold << std::endl;
    }
  ...
  unsigned int hash(unsigned int key, unsigned int i) { return hash2(key) * i + hash1(key); }

  // hash for indexing the table of local histories

  unsigned int hashlocal(void) { return u.pc >> 2; }
  ...
  std::pair<branch_info_mdp*, int> lookup(unsigned int pc, uint64_t full_pc)
  {

    // std::cout << "What is 15-295! " << tables[15][295] << std::endl;
    ...
    // if (full_pc == 281473768555768 && !spec_state)
    //   cout << __func__ << " " << full_pc << endl;
    ...
    if (num_filter) {
      ...
      // if (full_pc == 281473768555768 && !spec_state)
      //   cout << __func__ << "  ansf " << ansf << endl;

      // always taken so far?

      bool atsf = f->seen_taken & !(f->seen_untaken);
      // if (full_pc == 281473768555768 && !spec_state)
      //   cout << __func__ << " atsf " << atsf << endl;
      ...
      if (ansf) {
        ...
        u.filtered = true;
        // if (full_pc == 281473768555768 && !spec_state)
        //   cout << __func__ << " u.pred " << u.m_prediction << endl;

        return std::pair{&u, -250};
      } else if (atsf) {
    ...
    // if (full_pc == 281473768555768 && !spec_state)
    //   cout << __func__ << " bestval " << bestval << endl;

    // use the static prediction?
    if (use_static) {
      u.prediction(false); // static prediction of not taken
      // if (full_pc == 281473768555768 && !spec_state)
      //   cout << __func__ << " use_static " << u.m_prediction << endl;
    } else {
      ...
      if (abs(u.yout) <= threshold) {
        ...
        // if (full_pc == 281473768555768 && !spec_state)
        //   cout << __func__ << " abs(u.yout) <= threshold " << u.m_prediction << endl;
      } else
      ...
      // if (full_pc == 281473768555768 && !spec_state)
      //   cout << __func__ << " full perceptron prediction " << u.m_prediction << endl;
    }

    return std::pair{&u, u.yout};
  }
  ...
  void train(bool taken)
  {
    ...
    int a = abs(u.yout);
    // std::cout << "abs(u.yout) " << a << std::endl;
    ...
    // std::cout << "After threshold " << tables[15][295] << std::endl;

    // if the branch was predicted incorrectly or the correct
    // prediction was weak, update the weights

    // std::cout << "Vals " << correct << " a " << a << " theta " << theta << std::endl;
    ...
    // std::cout << "After thetha " << tables[15][295] << std::endl;
    ...
    // std::cout << "After table " << tables[15][295] << std::endl;
  ...
  void update(branch_info_mdp* p, unsigned int target, bool taken, int type, uint64_t pc)
  {
    // cout << __func__ << " pc " << u.pc  << " spec " << spec_state << endl;

    // if (pc == 18446462598868397980)
    // << "What is 15-295! before " << tables[15][295] << std::endl;
    ...
    // std::cout << "after filter " << tables[15][295] << std::endl;
    if (do_train)
      train(taken);

      // std::cout << "after train " << tables[15][295] << std::endl;
...
#define RECORD_FILTERED_RECENCY 128
    ...
    // std::cout << "after path history " << tables[15][295] << std::endl;
    ...
    // std::cout << "after acyclic history " << tables[15][295] << std::endl;
    ...
    if (!u.filtered || (record_mask & RECORD_FILTERED_MOD)) {
      for (int ii = 0; ii < nmodpath_histories; ii++) {
        ...
        if (hpc % (i + 2) == 0) {
          ...
          // cout << __func__ << " modpath_histories[i][1] " << modpath_histories[i][1] << " by " << u.pc << " spec " << spec_state << endl;
          modpath_histories[i][0] = u.pc2;
        }
      }
    }
    // std::cout << "after modulo path history " << tables[15][295] << std::endl;
    ...
    // std::cout << "after blurry history " << tables[15][295] << std::endl;

    // record into modulo pattern history
    // cout << __func__ << " filtered? " << u.filtered << " record_mask " << record_mask << endl;
    ...
    if (!u.filtered || (record_mask & RECORD_FILTERED_MOD)) {
      for (int ii = 0; ii < nmodhist_histories; ii++) {
        ...
        if (hpc % (i + 2) == 0) {
          ...
          // cout << __func__ << " mod_histories[i][0] " << mod_histories[i][0] << " mod_histories[i][1] " << mod_histories[i][1] << " i " << i << " spec " <<
          // spec_state << " hashed_taken " << hashed_taken << endl;
        }
      }
    }
    // std::cout << "after pattern history " << tables[15][295] << std::endl;
    ...
    // std::cout << "after recency history " << tables[15][295] << std::endl;
    ...
    // std::cout << "after local history " << tables[15][295] << std::endl;

    // update last ghist bit, used to index filter

    last_ghist_bit = taken;
    // if (pc == 18446462598868397980)
    // std::cout << "What is 15-295! after " << tables[15][295] << std::endl;
  }
  ...
  void SaveProcessorState()
  {
    ...
    u.SaveStateUpdate();
    spec_my_seed = my_seed;

    for (int i = 0; i < MAX_FILTER; i++) {
      spec_filter_table[i] = filter_table[i];
    }

    for (int i = 0; i < MAX_LOCAL_HISTORIES; i++) {
      spec_local_histories[i] = local_histories[i];
    }

    spec_tc = tc;
    spec_occupancy = occupancy;

    for (int i = 0; i < MAX_TABLES; i++) {
      for (int j = 0; j < MAX_TABLE_SIZE; j++) {
        spec_tables[i][j] = tables[i][j];
      }
    }
    // std::cout << "Saving! " << tables[15][295] << std::endl;

    for (int i = 0; i < MAX_TABLES; i++) {
      for (int j = 0; j < MAX_TABLE_SIZE; j++) {
        for (int k = 0; k < n_sign_bits; k++)
          spec_sign_bits[i][j][k] = sign_bits[i][j][k];
      }
    }

    for (int i = 0; i < (MAX_GHIST / MAX_LG_TABLE_SIZE + 1); i++) {
      spec_ghist_words[i] = ghist_words[i];
    }

    for (int i = 0; i < MAX_TABLES; i++) {
      spec_mpreds[i] = mpreds[i];
    }

    for (int i = 0; i < MAX_ACYCLIC; i++) {
      for (int j = 0; j < 32; j++) {
        spec_acyclic_histories[i][j] = acyclic_histories[i][j];
        spec_acyclic2_histories[i][j] = acyclic2_histories[i][j];
      }
    }

    for (int i = 0; i < MAX_MOD; i++) {
      for (int j = 0; j < MAX_GHIST; j++) {
        spec_mod_histories[i][j] = mod_histories[i][j];
      }
    }

    for (int i = 0; i < MAX_MOD; i++) {
      for (int j = 0; j < MAX_PATHHIST; j++) {
        spec_modpath_histories[i][j] = modpath_histories[i][j];
      }
    }

    for (int i = 0; i < MAX_BLURRY; i++) {
      for (int j = 0; j < MAX_BLURRY2; j++) {
        spec_blurrypath_histories[i][j] = blurrypath_histories[i][j];
      }
    }

    for (int i = 0; i < MAX_ASSOC; i++) {
      spec_recency_stack[i] = recency_stack[i];
    }

    spec_last_ghist_bit = last_ghist_bit;

    for (int i = 0; i < MAX_PATHHIST; i++) {
      spec_path_history[i] = path_history[i];
    }

    spec_imli_counter1 = imli_counter1;
    spec_imli_counter2 = imli_counter2;
    spec_imli_counter3 = imli_counter3;
    spec_imli_counter4 = imli_counter4;

    for (int i = 0; i < MAX_MOD; i++) {
      spec_modhist_indices[i] = modhist_indices[i];
      spec_modpath_indices[i] = modpath_indices[i];
      spec_modpath_lengths[i] = modpath_lengths[i];
      spec_modhist_lengths[i] = modhist_lengths[i];
    }
  ...
  void RestoreProcessorState()
  {
    ...
    u.RestoreStateUpdate();

    my_seed = spec_my_seed;

    for (int i = 0; i < MAX_FILTER; i++) {
      filter_table[i] = spec_filter_table[i];
    }

    for (int i = 0; i < MAX_LOCAL_HISTORIES; i++) {
      local_histories[i] = spec_local_histories[i];
    }

    tc = spec_tc;
    occupancy = spec_occupancy;

    for (int i = 0; i < MAX_TABLES; i++) {
      for (int j = 0; j < MAX_TABLE_SIZE; j++) {
        tables[i][j] = spec_tables[i][j];
      }
    }

    // std::cout << "Restoring! " << spec_tables[15][295] << std::endl;

    for (int i = 0; i < MAX_TABLES; i++) {
      for (int j = 0; j < MAX_TABLE_SIZE; j++) {
        for (int k = 0; k < n_sign_bits; k++)
          sign_bits[i][j][k] = spec_sign_bits[i][j][k];
      }
    }

    for (int i = 0; i < (MAX_GHIST / MAX_LG_TABLE_SIZE + 1); i++) {
      ghist_words[i] = spec_ghist_words[i];
    }

    for (int i = 0; i < MAX_TABLES; i++) {
      mpreds[i] = spec_mpreds[i];
    }

    for (int i = 0; i < MAX_ACYCLIC; i++) {
      for (int j = 0; j < 32; j++) {
        acyclic_histories[i][j] = spec_acyclic_histories[i][j];
        acyclic2_histories[i][j] = spec_acyclic2_histories[i][j];
      }
    }

    for (int i = 0; i < MAX_MOD; i++) {
      for (int j = 0; j < MAX_GHIST; j++) {
        mod_histories[i][j] = spec_mod_histories[i][j];
        // if (i == 0 && j == 1) {
        //   cout << "restoring! " << mod_histories[0][1] << " spec " << spec_mod_histories[0][1] << endl;
        // }
      }
    }
    ...
    for (int i = 0; i < MAX_MOD; i++) {
      for (int j = 0; j < MAX_PATHHIST; j++) {
        modpath_histories[i][j] = spec_modpath_histories[i][j];
      }
    }

    for (int i = 0; i < MAX_BLURRY; i++) {
      for (int j = 0; j < MAX_BLURRY2; j++) {
        blurrypath_histories[i][j] = spec_blurrypath_histories[i][j];
      }
    }

    for (int i = 0; i < MAX_ASSOC; i++) {
      recency_stack[i] = spec_recency_stack[i];
    }

    last_ghist_bit = spec_last_ghist_bit;

    for (int i = 0; i < MAX_PATHHIST; i++) {
      path_history[i] = spec_path_history[i];
    }

    imli_counter1 = spec_imli_counter1;
    imli_counter2 = spec_imli_counter2;
    imli_counter3 = spec_imli_counter3;
    imli_counter4 = spec_imli_counter4;

    for (int i = 0; i < MAX_MOD; i++) {
      modhist_indices[i] = spec_modhist_indices[i];
      modpath_indices[i] = spec_modpath_indices[i];
      modpath_lengths[i] = spec_modpath_lengths[i];
      modhist_lengths[i] = spec_modhist_lengths[i];
    }
    theta = spec_theta;
  }
...
public:
  MBP_PREDICTOR()
  {
    static int branchyxlat[] = {
        1, 3, 4, 5, 7, 8, 9, 11, 12, 14, 15, 17, 19, 21, 23, 25, 27, 29, 32, 34, 37, 41, 45, 49, 53, 58, 63, 69, 76, 85, 94, 106,
    };
    static int xlat4[] = {
...
#ifdef ALT_SIZE_8KB
    ex = new multiperspective_perceptron(good3_mbp, 16, 8192 * 8 + 2048,
...
#endif
#ifdef ALT_SIZE_64KB
    ex = new multiperspective_perceptron(big, 37, 65536 * 8 + 2048,
  ...
  void TrackOtherInst(uint64_t PC, uint8_t opType, bool branchDir, uint64_t branchTarget);
  ...
  bool spec_last_prediction = false;
};

inline bool MBP_PREDICTOR::GetPrediction(uint64_t PC)
{
  // std::cout << "What is 15-295! before " << ex->tables[15][295] << std::endl;

  // if (!ex->spec_state)
  //   cout << __func__ << " pc " << PC << " ex->mod_histories[0][1] " << ex->mod_histories[0][1];
  auto u = ex->lookup(PC, PC);
  last_prediction = u.first->prediction();
  // if (!ex->spec_state)
  // cout << " After " << ex->mod_histories[0][1] << endl;
  // std::cout << __func__ << " pc " << PC << " pred " << last_prediction << " spec " << ex->spec_state << std::endl;

  // std::cout << "What is 15-295! after " << ex->tables[15][295] << std::endl;

  return last_prediction;
}

inline void MBP_PREDICTOR::UpdatePredictor(uint64_t PC, uint8_t opType, bool resolveDir, uint64_t branchTarget)
{
  if (opType == BRANCH_CONDITIONAL)
    ex->update(u, branchTarget, resolveDir, opType, PC);
  else
    ex->nonconditional_branch(PC);
}

inline void MBP_PREDICTOR::TrackOtherInst(uint64_t PC, uint8_t opType, bool resolveDir, uint64_t branchTarget) { ex->nonconditional_branch(PC); }

inline void MBP_PREDICTOR::speculative_begin(uint64_t PC, uint8_t opType, bool resolveDir, uint64_t branchTarget)
{
  assert(!ex->spec_state);
  ex->spec_state = true;
  ex->SaveProcessorState();

  spec_last_prediction = last_prediction;

  UpdatePredictor(PC, opType, !resolveDir, branchTarget);

  // u->SaveState();
  // cout << " After " << ex->mod_histories[0][1] << endl;
}
inline void MBP_PREDICTOR::speculative_end()
{
  assert(ex->spec_state);
  // cout << __func__ << " ex->mod_histories[0][1] " << ex->mod_histories[0][1] << endl;
  ex->spec_state = false;
  ex->RestoreProcessorState();

  last_prediction = spec_last_prediction;
