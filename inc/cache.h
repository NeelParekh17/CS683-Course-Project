...
class CACHE : public champsim::operable, public MemoryRequestConsumer, public MemoryRequestProducer
{
  ...
  void update_dib_from_prefetch(const PACKET& pref_entry);
...
public:
  struct ip_in_line_entry {
    uint64_t ip = 0;
    uint64_t br_type = 0;
  };

  struct uop_mshr_entry {
    uint64_t cache_line_addr = 0;
    std::vector<ip_in_line_entry> ips_in_line;
    bool is_valid = false;
    uint64_t by_instr = 0;
    ...
    bool h2p_hit = false;
    uint64_t ip = 0;
    bool added_to_pq = false;
    bool btb_checked = false;
    bool by_correct_h2p = false;
    PACKET pq_pkt;
    deque<uint64_t> predicted_taken_branches;

    // ADDED THIS:
    bool is_alternate_path = false;  // Track if this MSHR is for alternate path
  };

  std::deque<uop_mshr_entry> UOP_CACHE_MSHR;
  ...
  struct NonTranslatingQueues : public champsim::operable {
    ...
    {
      get_profiler_ptr->m_PQ_SIZE = pq_size;
      get_profiler_ptr->m_RQ_SIZE = rq_size;
      get_profiler_ptr->m_WQ_SIZE = wq_size;
      get_profiler_ptr->m_PTWQ_SIZE = ptwq_size;
    }
  ...
  struct prefetch_decode_entry {
    ...
    // ADDED THIS:
    bool is_alternate_path = false;  // Track if from alternate path
  };

  std::deque<prefetch_decode_entry> cache_pref_decode_buffer;

  uint32_t cpu = 0;
  const std::string NAME;
  uint64_t last_ip_to_pref = 0;
  ...
  std::deque<uint64_t> uop_pref_addrs;
  ...
  std::pair<bool, bool> is_hit(uint64_t ip)
  {
    for (auto& entries : block) {
      if ((entries.v_address >> LOG2_BLOCK_SIZE) == (ip >> LOG2_BLOCK_SIZE)) {
        return std::pair{true, entries.prefetch};
      }
    }
    return {false, false};
  }

  void check_tag(uint64_t ip, uint64_t id, bool all_uop_hit);
  ...
  int prefetch_line_pkt(PACKET& pf_packet);
...
#include "cache_modules.inc"
  ...
  {
    get_profiler_ptr->m_OFFSET_BITS = offset_bits;
    get_profiler_ptr->l1ci_sets = v2;
    get_profiler_ptr->l1ci_ways = v3;
    get_profiler_ptr->l1ci_mshr = v8;
  }
};
