#ifndef _PREDICTOR_64KB_H_
#define _PREDICTOR_64KB_H_
...
#define IMNB 2
  ...
  int IMm[IMNB] = {10, 4};
  int spec_IMm[IMNB] = {10, 4};
  int8_t IMGEHLA[IMNB][(1 << LOGIMNB)] = {{0}};
  int8_t spec_IMGEHLA[IMNB][(1 << LOGIMNB)] = {{0}};

  int8_t* IMGEHL[IMNB];
  int8_t* spec_IMGEHL[IMNB];

  long long IMHIST[256];
  long long spec_IMHIST[256];
...
#define LOGPNB 9 // 1 1K + 2 * 512-entry tables
  int Pm[PNB] = {25, 16, 9};
  int spec_Pm[PNB] = {25, 16, 9};
  int8_t PGEHLA[PNB][(1 << LOGPNB)] = {{0}};
  int8_t spec_PGEHLA[PNB][(1 << LOGPNB)] = {{0}};
  int8_t* PGEHL[PNB];
  int8_t* spec_PGEHL[PNB];

  // first local history
#define LOGLNB 10 // 1 1K + 2 * 512-entry tables
#define LNB 3
  int Lm[LNB] = {11, 6, 3};
  int spec_Lm[LNB] = {11, 6, 3};
...
#define INDLOCAL ((PC ^ (PC >> 2)) & (NLOCAL - 1))
  ...
  long long spec_L_shist[NLOCAL]; // local histories

  // second local history
#define LOGSNB 9 // 1 1K + 2 * 512-entry tables
#define SNB 3
  int Sm[SNB] = {16, 11, 6};
  int spec_Sm[SNB] = {16, 11, 6};
  int8_t SGEHLA[SNB][(1 << LOGSNB)] = {{0}};
  int8_t spec_SGEHLA[SNB][(1 << LOGSNB)] = {{0}};

  int8_t* SGEHL[SNB];
  int8_t* spec_SGEHL[SNB];
...
#define INDSLOCAL (((PC ^ (PC >> 5))) & (NSECLOCAL - 1))
  ...
  long long spec_S_slhist[NSECLOCAL];
...
#define INDUPDS ((PC ^ (PC >> 2)) & ((1 << (LOGSIZEUPS)) - 1))
  ...
  int8_t spec_WS[(1 << LOGSIZEUPS)];
  int8_t spec_WT[(1 << LOGSIZEUPS)];
  int8_t spec_WP[(1 << LOGSIZEUPS)];
  int8_t spec_WI[(1 << LOGSIZEUPS)];
  int8_t spec_WIM[(1 << LOGSIZEUPS)];
...
#define TBITS 8 // minimum width of the tags  (low history lengths), +4 for high history lengths
  ...
  bool spec_NOSKIP[NHIST + 1]; // to manage the associativity for different history lengths
  bool spec_LowConf;
  bool spec_HighConf;
...
#define INDUSEALT (((((HitBank - 1) / 8) << 1) + AltConf) % (SIZEUSEALT - 1))
  ...
  folded_history spec_ch_i[NHIST + 1];
  folded_history ch_t[2][NHIST + 1]; // utility for computing TAGE tags
  folded_history spec_ch_t[2][NHIST + 1];
...
#define LOOPTAG 10         // tag width in the loop predictor
  ...
  void reinit()
  {
    ...
    spec_gtable[1] = new gentry[NBANKLOW * (1 << LOGG)];

    SizeTable[1] = NBANKLOW * (1 << LOGG);

    gtable[BORN] = new gentry[NBANKHIGH * (1 << LOGG)];
    spec_gtable[BORN] = new gentry[NBANKHIGH * (1 << LOGG)];
  ...
  void Tagepred(uint64_t PC)
  {
...

    {
      ...
      // if (!spec_state && PC == 18446462598881117280) {
      //   cout << "BIM alt " << PC << " " << alttaken << " BI " << BI << endl;
      // }
    }
    ...
    // if (!spec_state && PC == 18446462598881117280) {
    //   cout << "HitBank " << HitBank << " AltBank " << AltBank << endl;
    // }
    ...
    if (HitBank > 0) {
      if (AltBank > 0) {
        ...
        // if (!spec_state && PC == 18446462598881117280) {
        //   cout << "altbank alt " << PC << " " << alttaken << " AltBank " << AltBank << " GI[AltBank] " << GI[AltBank] << " ctr "
        //        << int(gtable[AltBank][GI[AltBank]].ctr) << endl;
        // }
      ...
      // if (!spec_state && PC == 18446462598881117280) {
      //   cout << "tage_pred alt " << tage_pred << " LongestMatchPred " << LongestMatchPred << " alttaken " << alttaken << endl;
      // }
    }
  }
  ...
  bool GetPrediction(uint64_t PC)
  {
    ...
    // if (!spec_state && PC == 18446462598881117280) {
    //   cout << "tage alt " << PC << " " << pred_taken << endl;
    // }
    ...
    // if (PC == 18446462598881117280 && !spec_state) {
    //   cout << "loop alt " << pred_taken << endl;
    // }
#endif
    ...
    // if (PC == 18446462598881117280 && !spec_state) {
    //   cout << "LSUM 1 " << LSUM << endl;
    // }
    LSUM += (2 * ctr + 1);
    // if (PC == 18446462598881117280 && !spec_state) {
    //   cout << "LSUM 2 " << LSUM << endl;
    // }
    ctr = BiasBank[INDBIASBANK];
    LSUM += (2 * ctr + 1);
    // if (PC == 18446462598881117280 && !spec_state) {
    //   cout << "LSUM 3 " << LSUM << endl;
    // }
#ifdef VARTHRES
    ...
    // if (PC == 18446462598881117280 && !spec_state) {
    //   cout << "SCPRED alt " << pred_taken << " SCPRED " << SCPRED << " LSUM " << LSUM << endl;
    // }
    ...
    // if (PC == 18446462598881117280 && !spec_state) {
    //   cout << "Prediction alt " << pred_taken << endl;
    // }
    return pred_taken;
  }
  ...
  // TAGE_PREDICTOR_64KB UPDATE

  void UpdatePredictor(uint64_t PC, uint8_t opType, bool resolveDir, uint64_t branchTarget)
  {

    // cout << __func__ << "_begin PC " << PC << " ctr val " << int(gtable[2][437].ctr) << " spec " << spec_state << endl;
#ifdef SC
#ifdef LOOPPREDICTOR
    if (LVALID) {
...
#define OPTREMP
    ...
    // cout << __func__ << " PC " << PC << " ctr val " << int(gtable[2][437].ctr) << endl;

    // END TAGE_PREDICTOR_64KB UPDATE
  }
#define GINDEX                                                                                                                                           \
...
#define CONFLOOP 15
  ...
  void speculative_begin(uint64_t PC, uint8_t opType, bool resolveDir, uint64_t branchTarget)
  {
    ...
    for (int i = 0; i < IMNB; i++) {
      spec_IMm[i] = IMm[i];
      spec_IMGEHL[i] = IMGEHL[i];
      for (int j = 0; j < (1 << LOGIMNB); j++) {
        spec_IMGEHLA[i][j] = IMGEHLA[i][j];
      }
    }

    for (int i = 0; i < 256; i++) {
      spec_IMHIST[i] = IMHIST[i];
    }
    ...
    for (int i = 0; i < PNB; i++) {
      spec_Pm[i] = Pm[i];
      spec_PGEHL[i] = PGEHL[i];
      for (int j = 0; j < (1 << LOGPNB); j++) {
        spec_PGEHLA[i][j] = PGEHLA[i][j];
      }
    }
    ...
    for (int i = 0; i < SNB; i++) {
      spec_Sm[i] = Sm[i];
      spec_SGEHL[i] = SGEHL[i];
      for (int j = 0; j < (1 << LOGSNB); j++) {
        spec_SGEHLA[i][j] = SGEHLA[i][j];
      }
    }

    for (int i = 0; i < NSECLOCAL; i++) {
      spec_S_slhist[i] = S_slhist[i];
    }

    for (int i = 0; i < TNB; i++) {
      spec_Tm[i] = Tm[i];
      spec_TGEHL[i] = TGEHL[i];
      for (int j = 0; j < (1 << LOGTNB); j++) {
        spec_TGEHLA[i][j] = TGEHLA[i][j];
      }
    }

    for (int i = 0; i < NTLOCAL; i++) {
      spec_T_slhist[i] = T_slhist[i];
    }
    ...
    for (int i = 0; i < (NHIST + 1); i++) {
      spec_ch_i[i] = ch_i[i];
      spec_ch_t[0][i] = ch_t[0][i];
      spec_ch_t[1][i] = ch_t[1][i];
    }
    ...
    */
    ...
    // cout << "Saving " << int(gtable[2][437].ctr) << " spec " << int(spec_gtable[2][437].ctr) << endl;
  ...
  void speculative_end()
  {
    ...
    for (int i = 0; i < IMNB; i++) {
      IMm[i] = spec_IMm[i];
      IMGEHL[i] = spec_IMGEHL[i];
      for (int j = 0; j < (1 << LOGIMNB); j++) {
        IMGEHLA[i][j] = spec_IMGEHLA[i][j];
      }
    }

    for (int i = 0; i < 256; i++) {
      IMHIST[i] = spec_IMHIST[i];
    }
    ...
    for (int i = 0; i < PNB; i++) {
      Pm[i] = spec_Pm[i];
      PGEHL[i] = spec_PGEHL[i];
      for (int j = 0; j < (1 << LOGPNB); j++) {
        PGEHLA[i][j] = spec_PGEHLA[i][j];
      }
    }
    ...
    for (int i = 0; i < SNB; i++) {
      Sm[i] = spec_Sm[i];
      SGEHL[i] = spec_SGEHL[i];
      for (int j = 0; j < (1 << LOGSNB); j++) {
        SGEHLA[i][j] = spec_SGEHLA[i][j];
      }
    }

    for (int i = 0; i < NSECLOCAL; i++) {
      S_slhist[i] = spec_S_slhist[i];
    }

    for (int i = 0; i < TNB; i++) {
      Tm[i] = spec_Tm[i];
      TGEHL[i] = spec_TGEHL[i];
      for (int j = 0; j < (1 << LOGTNB); j++) {
        TGEHLA[i][j] = spec_TGEHLA[i][j];
      }
    }

    for (int i = 0; i < NTLOCAL; i++) {
      T_slhist[i] = spec_T_slhist[i];
    }
    ...
    for (int i = 0; i < (NHIST + 1); i++) {
      ch_i[i] = spec_ch_i[i];
      ch_t[0][i] = spec_ch_t[0][i];
      ch_t[1][i] = spec_ch_t[1][i];
    }
