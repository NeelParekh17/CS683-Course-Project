...
std::pair<uint8_t, extra_branch_info> O3_CPU::predict_branch(uint64_t pc, uint8_t level, uint8_t branch_type, uint8_t branch_taken, bool need_only_h2p)
{
  ...
  // if (level == 1)
  //std::cout << "Predict Branch " << level << "  ip " << std::hex << pc << " taken " << std::dec << (yout[cpu] >= 1) << std::endl;
  return { yout[cpu] >= 1,br_info };
}

void O3_CPU::last_branch_result(uint64_t pc, uint64_t branch_target, uint8_t taken, uint8_t branch_type)
{
  //std::cout << "Update Branch ip " << std::hex << pc << std::dec << " taken " << int(taken) << std::endl;
