...
{
  //cout << __func__ << "_histories " << targetpc << endl;
#ifdef PC_SHIFT
...
bool
batage::predict(uint32_t pc, histories& p, uint64_t ip)
{
#ifdef PC_SHIFT
  pc ^= pc >> PC_SHIFT;
#endif
  ...
  for (int i = 0; i < NUMG; i++) {
    ...
    if (ip == 4224152) {
      cout << "gi[i] " << gi[i] << " getg(i).tag " << getg(i).tag << " p.gtag(pc, i) " << p.gtag(pc, i) << endl;
    }
