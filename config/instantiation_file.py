...
cpu_fmtstr = '{{{index}, {frequency}, {{{DIB[sets]}, {DIB[ways]}, {{champsim::lg2({DIB[window_size]})}}, {{champsim::lg2({DIB[window_size]})}}}},{DIB[sets]}, {DIB[ways]}, {DIB[window_size]}, {ifetch_buffer_size}, {dispatch_buffer_size}, {decode_buffer_size}, {rob_size}, {lq_size}, {sq_size}, {fetch_width}, {decode_width}, {dispatch_width}, {scheduler_size}, {execute_width}, {lq_width}, {sq_width}, {retire_width}, {mispredict_penalty}, {decode_latency}, {dispatch_latency}, {schedule_latency}, {execute_latency}, &{L1I}, {L1I}.MAX_TAG, &{L1D}, {L1D}.MAX_TAG, {branch_enum_string}, {btb_enum_string}}}'
...
def get_instantiation_lines(cores, caches, ptws, pmem, vmem):
    ...
    fill_levels = itertools.chain(*(enumerate(c['name'] for c in util.iter_system(
        memory_system, cpu[name])) for cpu, name in itertools.product(cores, ('ITLB', 'DTLB', 'L1I', 'L1D'))))
    fill_levels = sorted(fill_levels, key=operator.itemgetter(1))
    fill_levels = ({'name': n, '_fill_level': max(
        l[0] for l in fl)} for n, fl in itertools.groupby(fill_levels, key=operator.itemgetter(1)))
    memory_system = util.combine_named(fill_levels, memory_system.values())

    # Remove name index
    memory_system = sorted(memory_system.values(
    ), key=operator.itemgetter('_fill_level'), reverse=True)
    ...
    for elem in memory_system:
        if 'pscl5_set' in elem:
            ...
            yield queue_fmtstr.format(
                _type='TranslatingQueues' if elem.get(
                    '_needs_translate') else 'NonTranslatingQueues',
                **elem)
            yield cache_fmtstr.format(
                prefetch_activate_mask=' | '.join(
                    f'(1 << {t})' for t in elem['prefetch_activate'].split(',')),
                repl_enum_string=' | '.join(
                    f'CACHE::r{k}' for k in elem['_replacement_modnames']),
                pref_enum_string=' | '.join(
                    f'CACHE::p{k}' for k in elem['_prefetcher_modnames']),
                **elem)

    yield from ('O3_CPU ' + cpu['name'] + cpu_fmtstr.format(
                branch_enum_string=' | '.join(
                    f'O3_CPU::b{k}' for k in cpu['_branch_predictor_modnames']),
                btb_enum_string=' | '.join(
                    f'O3_CPU::t{k}' for k in cpu['_btb_modnames']),
