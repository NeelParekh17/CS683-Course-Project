...
def default_dir(dirnames, f):
    return next(filter(os.path.exists, map(norm_dirname, itertools.chain(
        (os.path.join(dirname, f)
         for dirname in dirnames),  # Prepend search paths
        (f,)  # Interpret as file path
    ))))


def get_module_data(names_key, paths_key, values, directories, get_func):
    namekey_pairs = itertools.chain(*(zip(c[names_key], c[paths_key], itertools.repeat(
        c.get('_is_instruction_prefetcher', False))) for c in values))
    data = util.combine_named(
        itertools.chain(*(default_modules(directory)
                        for directory in directories if os.path.exists(directory))),
        ({'name': name, 'fname': path, '_is_instruction_prefetcher': is_instr}
         for name, path, is_instr in namekey_pairs)
    )
...
def data_getter(prefix, module_name, funcs):
    return {
        ...
        'func_map': {k: '_'.join((prefix, module_name, k)) for k in funcs}
    }


def get_branch_data(module_name):
    return data_getter('bpred', module_name, ('initialize_branch_predictor', 'last_branch_result', 'predict_branch', 'br_speculative_begin', 'br_speculative_end', 'get_yout', 'save_histogram'))


def get_btb_data(module_name):
    return data_getter('btb', module_name, ('initialize_btb', 'update_btb', 'btb_prediction', 'btb_bb_size', 'update_bb_size', 'speculative_begin', 'speculative_end', 'get_branch_type', 'get_type'))
...
def discriminator_function_declaration(fname, rtype, args, attrs=[], classname=None):
    ...
    if classname is None:
        attrstring = '[[' + \
            ', '.join(itertools.chain(attrs, local_attrs)) + ']]'
        argstring = ', '.join(a[0] for a in args)
    else:
        attrstring = ''
        argstring = ', '.join((a[0]+' '+a[1]) for a in args)

    funcstring = ((classname + '::')
                  if classname is not None else '') + 'impl_' + fname
...
def get_branch_lines(branch_data):
    ...
    branch_variant_data = [
        ...
        ('last_branch_result', (('uint64_t', 'ip'), ('uint64_t', 'target'),
         ('uint8_t', 'taken'), ('uint8_t', 'branch_type'))),
         ('predict_branch', (('uint64_t', 'ip'),
         ('uint8_t', 'level'),('uint8_t', 'type'),('uint8_t', 'taken'),('bool', 'need_only_h2p')), 'std::pair<uint8_t, extra_branch_info>', '::take_last'),
        ('get_yout', (('uint64_t', 'ip'), ('uint8_t', 'level'),), 'int', 'std::bit_or'),
        ('br_speculative_begin', (('uint64_t', 'ip'), ('uint64_t', 'target'),
         ('uint8_t', 'taken'), ('uint8_t', 'branch_type'), ('uint8_t', 'level'))),
        ('br_speculative_end',),
        ('save_histogram', (('uint64_t', 'ip'),
         ('uint8_t', 'level'),), 'int', 'std::bit_or')
    ]

    return (
        itertools.chain(
            constants_for_modules(prefix, varname_size_name,
                                  branch_data.values()), ('',),
...
def get_btb_lines(btb_data):
    ...
    btb_variant_data = [
        ('initialize_btb',),
        ('update_btb', (('uint64_t', 'ip'), ('uint64_t', 'predicted_target'),
         ('uint8_t', 'taken'), ('uint8_t', 'branch_type'),('bool','alt_path'))),
        ('btb_prediction', (('uint64_t', 'ip'), ('uint8_t', 'level'),),
         'std::pair<std::pair<uint64_t, uint8_t>, extra_branch_info>', '::take_last'),
        ('btb_bb_size', (('uint64_t', 'ip'), ('uint8_t', 'level'),),
         'std::pair<uint64_t, uint64_t>', '::take_last'),
        ('update_bb_size', (('uint64_t', 'ip'), ('uint8_t', 'level'), ('uint64_t', 'bb_size'),
         ('bool', 'taken'), ('uint8_t', 'branch_type'), ('uint64_t', 'branch_hist'),)),
        ('speculative_begin',(('uint64_t', 'ip'), ('uint64_t', 'predicted_target'),
         ('uint8_t', 'taken'), ('uint8_t', 'branch_type'),('bool','alt_path'))),
        ('speculative_end',),
        ('get_branch_type', (('uint64_t', 'ip'),), 'uint8_t', 'std::bit_or'),
        ('get_type', (('uint64_t', 'ip'),), 'uint8_t', 'std::bit_or'),
    ]
...
def get_pref_lines(pref_data):
    ...
    pref_nonbranch_variant_data = [
        ...
        ('prefetcher_cache_fill', (('uint64_t', 'addr'), ('uint32_t', 'set'), ('uint32_t', 'way'), ('uint8_t',
         'prefetch'), ('uint64_t', 'evicted_addr'), ('uint32_t', 'metadata_in')), 'uint32_t', 'std::bit_xor'),
        ('prefetcher_cycle_operate',),
        ('prefetcher_final_stats',)
    ]

    pref_branch_variant_data = [
        ('prefetcher_branch_operate', (('uint64_t', 'ip'),
         ('uint8_t', 'branch_type'), ('uint64_t', 'branch_target')))
    ]
    ...
    return (
        itertools.chain(
            ...
            *(get_module_variant_declarations(fname, [v['func_map'][fname] for v in pref_data.values(
            )], *finfo) for fname, *finfo in pref_nonbranch_variant_data),

            # Establish functions that only matter to instruction prefetchers
            ('', '// Assert data prefetchers do not operate on branches'),
            *(mangled_prohibited_definitions(fname, [v['func_map'][fname] for v in pref_data.values() if not v.get(
                '_is_instruction_prefetcher')], *finfo) for fname, *finfo in pref_branch_variant_data),
...
def get_repl_lines(repl_data):
    ...
    repl_variant_data = [
        ...
        ('find_victim', (('uint32_t', 'triggering_cpu'), ('uint64_t', 'instr_id'), ('uint32_t', 'set'), ('const BLOCK*',
         'current_set'), ('uint64_t', 'ip'), ('uint64_t', 'full_addr'), ('uint32_t', 'type')), 'uint32_t', '::take_last'),
        ('update_replacement_state', (('uint32_t', 'triggering_cpu'), ('uint32_t', 'set'), ('uint32_t', 'way'),
